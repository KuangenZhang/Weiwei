...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_model", type=str, default='ke', help=" path to checkpoint model")
    ...
    parser.add_argument('--model_name', type=str, default='SimpleModel', help='chooses which model to use. [resnet18 | resnet34 | resnet50| resnet101 | resnet152]')
    ...
    optimizer = torch.optim.Adam(model.parameters(), betas=(opt.beta1, 0.999), lr=opt.lr)
    ...
    if opt.phase == 'train':
        ...
        for epoch in tqdm(range(opt.n_epochs + opt.n_epochs_decay)):
            ...
            loss_te, loss_c, X_te, y_te, y_tep, class_label, condition_tag = test_rg(model, train_loader, opt)
        ...
        test_loader =  data_prepared['data_loader_test']
        model.load_state_dict(torch.load("ke.pt"))
        loss_te, loss_c, X_te, y_te, y_tep, class_label, condition_tag = test_rg(model, test_loader, opt)
        print(loss_te)
